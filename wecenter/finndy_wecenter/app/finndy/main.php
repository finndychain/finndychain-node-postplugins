...
include AWS_PATH . '../app/finndy/functions.php';
...
class main extends AWS_CONTROLLER {
    ...
    public function index_action() {
        ...
        Header("Location:http://localhost/wecenter/");
    }
    ...
    public function details_action() {
        ...
        if ($_GET["type"] == "cate") {
            ...
            foreach ($cates as $value) {
                 $category[] = array('id' => $value['id'], 'name' => $value['title']);
            }
            if (!empty($category)) {
                echo json_encode(array('status'=>1,'msg'=>'','data'=>$category));exit();
            }else{
    ...
    public function version_action() {
        ...
        finndy_success($version);
    }

    //验证方法
    private function validation() {
        if (empty($_REQUEST['token'])) {
            finndy_fail(FINNDYERROR_INVALID_PWD, "password is empty", "TOKEN为空");
        }
        if (file_exists(AWS_PATH . '/config/finndy_config.php')) {
            require AWS_PATH . '/config/finndy_config.php';
        }
        ...
        if ($_REQUEST['token'] != $finndy_config["finndy_password"]) {
            finndy_fail(FINNDYERROR_INVALID_PWD, "password is wrong", "TOKEN填写错误");
        }
    }
    ...
    public function article_post_action() {
        ...
        if (empty($title)) {
            finndy_fail(FINNDYERROR_MISSING_FIELD, 'article_title is empty', "缺少文章标题");
        }
        if (empty($content)) {
            finndy_fail(FINNDYERROR_MISSING_FIELD, 'article_content is empty', "缺少文章内容");
        }
        ...
        if (empty($user_id)) {
            finndy_fail(FINNDYERROR_ERROR, 'user_id is empty', "请先设置文章作者");
        }
        ...
        // }
        $category_id = $_POST['article_categories'];
        ...
        if (empty($article_id)) {
            finndy_fail(FINNDYERROR_ERROR, 'insert failed', "文章插入失败");
        }
        ...
        finndy_success(array("url" => base_url() . "/" . G_INDEX_SCRIPT . "article/" . $article_id));
    }
    ...
    public function discussion_post_action() {
        ...
        if (empty($question)) {
            finndy_fail(FINNDYERROR_MISSING_FIELD, 'question_title is empty', "缺少问答标题");
        }
        ...
        if (empty($question_id)) {
            finndy_fail(FINNDYERROR_ERROR, 'insert fail', "问答插入失败");
        }
        ...
        if ($category_id) {
            $update_data = array(
                'category_id' => $_POST['default_forum']
            );
        ...
        finndy_success(array("url" => base_url() . "/" . G_INDEX_SCRIPT . "question/" . $question_id));
    }
    ...
    function set_user_avatar($user_id, $avatar) {
        $image_url = finndy_redirect_url($avatar);
